...
import { TrendingUp, Plus, Pencil, Trash2, Check, X, Lock, Briefcase, CalendarDays, Merge } from 'lucide-react';
...
// Member może edytować/kasować tylko swoje wpisy (createdBy === currentUserId)
// albo legacy bez autora (createdBy == null). Owner gospodarstwa może wszystko.
const canMutateEntry = (item, currentUserId, isOwner) =>
  isOwner || item?.createdBy == null || item.createdBy === currentUserId;

export const IncomeSection = ({ incomes, addIncome, updateIncome, deleteIncome, mergeIntoFixed = null, currentUserId = null, isOwner = false }) => {
  ...
  const mergeCandidatesFor = (income) =>
    mergeIntoFixed && canMutate(income) ? mergeCandidates(income, fixedIncomes, canMutate) : [];
  ...
  return (
    <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-4 sm:p-6 h-full flex flex-col min-h-0">
      ...
      <div className="finance-entry-list space-y-3">{sortedIncomes.length === 0 ? <p className="text-slate-500 text-center py-8">Brak przychodów w tym miesiącu</p> : sortedIncomes.map(income => (
        <div key={income.id} className={`finance-entry-row flex items-center justify-between p-4 rounded-xl border transition-all ${income.isFixed ? 'bg-emerald-500/10 border-emerald-500/30 hover:border-emerald-400/50' : 'bg-slate-700/30 border-slate-600/30 hover:border-teal-500/30'}`}>
          {editingId === income.id ? <div className="flex-1 flex flex-col gap-3"><div className="grid grid-cols-1 md:grid-cols-3 gap-3"><input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} className="px-3 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-emerald-500" /><input type="number" value={editAmount} onChange={(e) => setEditAmount(e.target.value)} className="px-3 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-emerald-500" step="0.01" /><input type="date" value={editDate} onChange={(e) => setEditDate(e.target.value)} className="px-3 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white focus:outline-none focus:border-emerald-500" /></div><div className="flex items-center justify-between"><button onClick={() => setEditIsFixed(!editIsFixed)} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-all ${editIsFixed ? 'bg-emerald-500/30 text-emerald-300' : 'bg-slate-700 text-slate-400'}`}>{editIsFixed ? <Lock className="w-3 h-3" /> : <Briefcase className="w-3 h-3" />}{editIsFixed ? 'Stały' : 'Zmienny'}</button><div className="flex gap-2"><button onClick={() => handleSaveEdit(income.id)} className="p-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg transition-all"><Check className="w-4 h-4" /></button><button onClick={() => setEditingId(null)} className="p-2 bg-slate-600 hover:bg-slate-500 text-white rounded-lg transition-all"><X className="w-4 h-4" /></button></div></div></div> : <><div className="flex-1 min-w-0"><div className="flex items-center gap-2 flex-wrap min-w-0">{income.isFixed && <Lock className="w-3.5 h-3.5 text-emerald-400 shrink-0" />}<EntryName name={income.name} />{income.isFixed && <span className="px-1.5 py-0.5 bg-emerald-500/20 text-emerald-400 text-xs rounded">stały</span>}<MergeIntoFixedControl candidates={mergeCandidatesFor(income)} onPick={(fixed) => setMergeTarget({ entry: income, fixed })} /></div>{income.date && <p className="text-sm text-slate-400 flex items-center gap-1 mt-1"><CalendarDays className="w-3 h-3" />{formatDate(income.date)}</p>}</div><div className="flex items-center gap-2 sm:gap-4 shrink-0"><span className={`font-semibold whitespace-nowrap ${income.isFixed ? 'text-emerald-400' : 'text-teal-400'}`}>{formatCurrency(income.amount)}</span>{canMutateEntry(income, currentUserId, isOwner) ? <div className="flex gap-1"><button onClick={() => handleEdit(income)} className="p-2 text-slate-400 hover:text-white hover:bg-slate-600 rounded-lg transition-all"><Pencil className="w-4 h-4" /></button><button type="button" onClick={() => setDeleteTarget({ id: income.id, name: income.name, amountLabel: formatCurrency(income.amount) })} className="p-2 text-slate-400 hover:text-rose-400 hover:bg-rose-500/20 rounded-lg transition-all"><Trash2 className="w-4 h-4" /></button></div> : <ReadOnlyEntryActions kind="income" />}</div></>}
        </div>
      ))}</div>
      <ConfirmDialog
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (mergeTarget) mergeIntoFixed('income', mergeTarget.entry.id, mergeTarget.fixed.id);
          setMergeTarget(null);
        }}
        title="Scalić z przychodem stałym?"
        description={mergeTarget ? mergeDescription(mergeTarget, 'przychód stały') : ''}
