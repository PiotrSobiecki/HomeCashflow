...
export const EnergyReportExport = ({ devices }) => {
  ...
  const [selected, setSelected] = useState(() => new Set(devices.map((d) => d.id)))
  ...
  const handleExport = async (mode) => {
    ...
    if (validationError) { setMessage({ ok: false, text: validationError }); return }
    setBusy(mode)
    setMessage(null)
    try {
      const deviceIds = selected.size === devices.length ? undefined : [...selected]
      ...
      if (mode === 'download') {
        ...
        const { to: email } = await emailEnergyReport({ pdfBase64: pdfToBase64(doc), from, to })
  ...
  return (
    <div className="bg-gradient-to-br from-emerald-500/10 to-slate-800/50 border border-emerald-500/30 rounded-2xl p-6">
      ...
      <p className="text-sm text-slate-400 mb-5">Wybierz okres i urządzenia, potem pobierz PDF albo wyślij go na swój email.</p>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Okres */}
        <div className="bg-slate-900/40 border border-slate-700/50 rounded-xl p-4">
          <p className="text-[11px] font-medium uppercase tracking-wider text-emerald-400 mb-3">Okres</p>
          ...
          <div className="flex flex-wrap gap-1.5">
            {PRESETS.map((p) => (
              <button
                key={p.days} type="button" onClick={() => applyPreset(p.days)}
        ...
        <div className="bg-slate-900/40 border border-slate-700/50 rounded-xl p-4">
          <p className="text-[11px] font-medium uppercase tracking-wider text-emerald-400 mb-3">
            Urządzenia <span className="text-slate-500 normal-case tracking-normal">({selected.size}/{devices.length})</span>
          </p>
          <div className="flex flex-wrap gap-2">
            {devices.map((d) => (
              ...
              >
                <input
                  type="checkbox" checked={selected.has(d.id)} onChange={() => toggleDevice(d.id)}
                  className="sr-only"
                />
        ...
        {/* Eksport */}
        <div className="bg-slate-900/40 border border-slate-700/50 rounded-xl p-4">
          <p className="text-[11px] font-medium uppercase tracking-wider text-emerald-400 mb-3">Eksport</p>
          ...
          {message && (
            <p className={`flex items-start gap-1.5 mt-3 text-xs ${message.ok ? 'text-emerald-400' : 'text-rose-400'}`}>
              {message.ok ? <Check className="w-3.5 h-3.5 shrink-0 mt-0.5" /> : <X className="w-3.5 h-3.5 shrink-0 mt-0.5" />}
              {message.text}
            </p>
          )}
        </div>
